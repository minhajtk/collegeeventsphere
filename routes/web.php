...
// 6. Hosting Helper Utility for Shared Hosting (InfinityFree)
Route::get('/hosting-setup', function (\Illuminate\Http\Request $request) {
    $key = $request->query('key');
    if ($key !== 'eventsphere2026') {
        return response('Access Denied. Please provide ?key=eventsphere2026', 403);
    }
    ...
    try {
        ...
        return response('<div style="font-family:sans-serif; text-align:center; padding:3rem;"><h2 style="color:#22c55e;">EventSphere InfinityFree Setup Complete!</h2><p>Storage symlink created and framework caches cleared successfully.</p><a href="/" style="display:inline-block; margin-top:1rem; padding:0.6rem 1.2rem; background:#000; color:#fff; text-decoration:none; border-radius:6px;">Go to Public Home</a></div>', 200);
    } catch (\Throwable $e) {
